...
def save_to_excel(artist_first_scrobble, username, filename=None):
    if filename is None:
        filename = f'result/{username}_1st_scrobbles.xlsx'
...
async def main():
    ...
    logging.info("Fetching scrobbles...")
    try:
        scrobbles = await get_scrobbles(username)
        logging.info("Processing scrobbles...")
        artist_first_scrobble = get_first_scrobble_dates(scrobbles)
        logging.info("Saving to Excel...")
        save_to_excel(artist_first_scrobble, username)
        logging.info(f"First scrobbled dates saved to 'result/{username}_1st_scrobbles.xlsx'")
